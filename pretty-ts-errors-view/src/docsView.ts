...
export class DocsViewViewProvider implements vscode.WebviewViewProvider {
   ...
   private static readonly pinnedContext =
      'docsView.documentationView.isPinned';
...
<link href="${codiconsUri}" rel="stylesheet" />
   ...
   private async update(ignoreCache = false) {
      ...
      const updatePromise = (async () => {
         const html = await this.getHtmlContentForActiveEditor(
      ...
      await Promise.race([
         ...
         new Promise<void>((resolve) => setTimeout(resolve, 250)).then(() => {
            ...
            return vscode.window.withProgress(
               { location: { viewId: DocsViewViewProvider.viewType } },
               () => updatePromise
            );
         }),
      ]);
   ...
   private async getHtmlContentForActiveEditor(
      ...
      const diagnostics = this.getDiagnosticsAtCurrentPositionInEditor(editor);
      ...
      return diagnostics?.length
         ? this._renderer.render(editor.document, diagnostics)
         : '';
   }

   private getDiagnosticsAtCurrentPositionInEditor(editor: vscode.TextEditor) {
      const diagnostics = vscode.languages.getDiagnostics(editor.document.uri);
      const diagnosticsAtPosition = diagnostics.filter((diagnostic) =>
         diagnostic.range.contains(editor.selection.active)
      );

      return diagnosticsAtPosition;
   }
   ...
   private updateConfiguration() {
      ...
      this._updateMode =
         config.get<UpdateMode>('documentationView.updateMode') ||
         UpdateMode.Live;
   }
}
