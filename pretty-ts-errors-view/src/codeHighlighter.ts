...
export class CodeHighlighter {
   ...
   public async getHighlighter(
      document: vscode.TextDocument
   ): Promise<(code: string, language: string) => string> {
         ...
         if (language && highlighter) {
            try {
               ...
               if (languageId) {
                  return (
                     '<pre class="code">' +
                     highlighter.codeToHtml!(code, languageId) +
                     '</pre>'
                  );
               }
            } catch (err) {
      ...
      return (code: string, inputLanguage: string): string => {
         ...
         return code + `<br>language: ${language}`;
      };
   }
...
function getLanguageId(inId: string): string | undefined {
   for (const language of languages) {
      if (
         inId === language.name ||
         language.identifiers.some((langId) => inId === langId)
      ) {
...
async function getDefaultForeground(uri: vscode.Uri): Promise<string> {
   try {
      ...
      const json = json5parse(contents);
...
const languages = [
   ...
   // syntax highlighting.
   {
      ...
      source: ['text.html.basic', 'text.html.php', 'source.php'],
   },
   {
      name: 'sql',
      language: 'sql',
   ...
   },
   {
      name: 'ocaml',
      language: 'ocaml',
      identifiers: ['ml', 'mli', 'eliom', 'eliomi'],
      source: 'source.ocaml.interface',
   },
